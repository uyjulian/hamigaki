// sprite_info.hpp: sprite infomration for action_game
...
struct sprite_info_set
{
    ...
    const sprite_group& get_group(boost::uint32_t form) const
    {
        typedef std::map<boost::uint32_t,sprite_group> table_t;
        table_t::const_iterator pos = groups.find(form);
        if (pos == groups.end())
