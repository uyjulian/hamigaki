// aiff_file_test.cpp: test case for aiff_source/aiff_sink
...
void aiff_file_test_aux(const audio::pcm_format& fmt, float freq)
{
    ...
    io::copy(
        audio::widen<float>(audio::make_aiff_file_source(tmp)),
    ...
    for (std::size_t i = 0; i < sizeof(rates)/sizeof(rates[0]); ++i)
    {
        for (std::size_t j = 0; j < sizeof(types)/sizeof(types[0]); ++j)
        {
            for (unsigned channels = 1; channels <= 2; ++channels)
            {
                ...
                aiff_file_test_aux(fmt, calc_frequency(note));
            }
            note = next_note(note);
        }
    }
...
{
    ut::test_suite* test = BOOST_TEST_SUITE("aiff file test");
    test->add(BOOST_TEST_CASE(&aiff_file_test));
    return test;
}
