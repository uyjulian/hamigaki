...
namespace
{
  ...
  string url_prefix_dir_view( ".." );
  string url_prefix_checkout_view( ".." );
...
const fs::path find_bin_path(const string& relative)
{
  ...
  {
    bin_path = locate_root / "bin/hamigaki" / relative;
    if (!fs::exists(bin_path))
    {
      ...
      if (!fs::exists(bin_path))
      {
        ...
        if (!fs::exists(bin_path))
        {
          bin_path = fs::path( locate_root / "bin/hamigaki/libs" /
...
#else
    relative.erase( 0, fs::initial_path().branch_path().string().size()+1 );
#endif
...
int cpp_main( int argc, char * argv[] ) // note name!
{
  ...
  report << "<html>\n"
          "<head>\n"
          "<title>Hamigaki Compiler Status Automatic Test</title>\n"
          "</head>\n"
          "<body bgcolor=\"#ffffff\" text=\"#000000\">\n"
          "<table border=\"0\">\n"
          "<tr>\n"
          "<td><img border=\"0\" src=\"../hamigaki.png\" width=\"96\" "
          "height=\"72\"></td>\n"
          "<td>\n"
